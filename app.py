...
def start():
    flash(
        ...
        only the province of Liège is covered."
    )
...
def display(searchterm="", range_value=""):
    ...
    cached = (
        f"./templates/maps/{instance.x}x{instance.y}y{instance.boundary}.html"
    )
    ...
    for feature in features:
        try:
            hits.append(
                {
                    "Length": f"{round(feature['SHAPE_Length'], 2)} m.",
                    "Area": f"{round(feature['SHAPE_Area'], 2)} m.²",
                    "Ceiling Height": f"{round(feature['E_TOIT'], 2)} m.",
                }
            )
        except KeyError:
            pass
    return render_template(
        ...
        h2={"Select the RED CONTOUR": "for building infos 🏠"},
        html_map=html_map,
        hits=hits,
    )
